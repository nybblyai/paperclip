...
- Run the next real-use validation slice from section 8 by covering either a blocked/resume route-path scenario on the same issue update/activity contract or a specialist follow-on reroute (`Ork -> Main` or `Main -> Stitch`) that extends the now-landed handoff plus needs-human-attention route validations
...
- Main remaining gap is broader real-use OpenClaw integration validation on top of the now-verified direct service seam and route seam, including at least one additional specialist scenario plus blocked/resume coverage
