...
- Run one real tracked-work dry run through the existing Paperclip issue surfaces so `Main` can hand off to a specialist, the specialist can either escalate or block/resume, and the operator can confirm the current owner, next step, and latest meaningful history without rereading transcript chatter
...
- Main remaining gaps are the real tracked-work usability proof above plus actual Telegram behavior verification on top of the now-verified route seam
