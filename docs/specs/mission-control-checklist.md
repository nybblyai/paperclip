...
- Run the blocked/resume route-path scenario on the same issue update/activity contract so the remaining real-use state-transition gap closes without inventing any new routing or control-plane model
...
- Main remaining gaps are actual Telegram behavior verification plus broader real-use OpenClaw integration validation on top of the now-verified direct service seam and route seam, with blocked/resume coverage and real tracked-work usability still open
