...
- Implement and verify the new Telegram emergence contract with a small summary builder or fixture-driven projection that proves summary mode vs transparent mode against mixed handoff, blocker, approval, and chatter-heavy issue history
...
- Main remaining gaps are actual Telegram behavior verification plus broader real-use OpenClaw integration validation on top of the now-verified direct service seam and route seam, including at least one additional specialist scenario plus blocked/resume coverage
