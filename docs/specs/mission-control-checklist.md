...
- Run the next real-use validation slice from section 8 by extending beyond the `Main -> Ork` route-path handoff into either a specialist follow-on reroute (`Ork -> Main` or `Main -> Stitch`) or a needs-human-attention scenario that proves the same broader issue route/history path stays compact and durable
...
- Main remaining gap is broader real-use OpenClaw integration validation on top of the now-verified direct service seam and route seam, including at least one additional specialist/human-attention scenario plus blocked/resume coverage
