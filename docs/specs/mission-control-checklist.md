...
- Verify Telegram summary mode versus transparent mode against the now-landed mission-control activity/history fixtures so emergence behavior is proven on real structured state instead of remaining spec-only
...
- Main remaining gaps are actual Telegram behavior verification plus broader real-use OpenClaw integration validation on top of the now-verified direct service seam and route seam, with real tracked-work usability still open
